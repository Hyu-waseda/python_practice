...
<title>予約一覧</title>
	<body>
		<h2>予約一覧</h2>
		<p><a href="/search/{{str(data[0]["user_id"])}}">条件付きで予約検索</a></p>
		<p>会議室A</p>
		<table border="2" style="border-collapse: collapse">
			<tr>
				<td width="150">年月日</td><td width="100">利用時間</td><td width="150">利用目的</td><td width="95">利用者名</td>
				<td></td>
			</tr>
		</table>
		%for d in data:
		...
		%for d in data:
			%if len(d) == 8 and d["room"] == "会議室C":
				%if d["user_id"] == data[0]["user_id"] or data[0]["user_id"] == "admin":
					<table border="1" style="border-collapse: collapse">
						<tr>
							<td width="150">{{d["year"]}}年{{d["month"]}}月{{d["day"]}}日</td><td width="100">{{d["hour"]}}時〜{{d["hour"]+1}}時</td><td width="150">{{d["title"]}}</td><td width="95">{{d["name"]}}</td>
							<td width="50">
								<form method="POST" action="/change/{{str(data[0]["user_id"])}}">
									<input type="hidden" name="title" value = {{d["title"]}} required/>
									<input type="hidden" name="name" value = {{d["name"]}} required/>
